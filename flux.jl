#flux.jl solves the FBA problem by constraining a LP problem
# it uses glpk as a linear programming tool kit

using GLPK

"""
Inputs:
`stoichMat` - stoichiometric_matrix (M x R)
`reactionBounds` - bounds on Vmin and Vmax for each reaction (R x 2)
`speciesBounds` - bounds on species concentration (all zeros here at SS) (M x 2)
`objective` - Vector indicating desired flux to solve for (R x 1) 
`minFlag` - Indicates if problem is minimization (Boolean)
...
function calculate_optimal_flux_distribution(stoichMat::Array{Float64,2},reactionBounds::Array{Float64,2},speciesBounds::Array{Float64,2},objective::Array{Float64,2},minFlag::Bool=true)
    ...
    for (j,k) in enumerate(objective)
